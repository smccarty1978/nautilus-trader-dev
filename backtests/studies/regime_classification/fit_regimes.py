"""Phase 2 — fit unsupervised regime models on 1m features.

Discipline:
  - StandardScaler fitted on IS 2020-2022 only, applied to all years.
  - Each model fitted on IS rows (no NaN), with random_state=42.
  - State labels predicted across the entire 2020-2026 window.
  - Tests all (model, n_states) combinations.

Models:
  - GaussianHMM (full covariance)
  - GaussianMixture (full covariance, EM)
  - KMeans (MiniBatchKMeans for speed)
  - BayesianGaussianMixture (optional)

State counts: 3, 4, 5, 6.

Output: studies/regime_classification/results/states_{product}_1m.parquet
  with columns hmm_K, gmm_K, kmeans_K for K in {3,4,5,6} added to features
  parquet.
"""
...
def main():
    t0 = time.time()
    in_p = OUT / f"features_{PRODUCT.lower()}_1m.parquet"
    df = pd.read_parquet(in_p)
    print(f"Loaded {len(df):,} rows from {in_p.name}")

    # Drop rows with any NaN in features (warmup, gaps)
    ...
    out_p = OUT / f"states_{PRODUCT.lower()}_1m.parquet"
    df.to_parquet(out_p)
    print(f"\nsaved {out_p}")

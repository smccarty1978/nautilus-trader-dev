"""Deployable bar1_confirm + 5m macro HMM state overlay (corrected anchor).
...
Entry    : open of the 1s bar at T + 60s (i.e. bar1 close moment ≈ open of bar2).
Exits    :
  A. +1.0 / -1.0 ATR first-touch on 1s bars (from bar1-close-entry).
  B. Regime-exit (already in nt_regime_exit_nq.parquet, bar1-close anchor).
States   : 5-minute macro states looked up strictly causally:
  - state_at_flip = state of the latest 5m bar fully closed at flip time T.
  - state_at_bar1 = state of the latest 5m bar fully closed at bar1 close T + 60s.
...
PRODUCT_DATA = {
    "NQ": {"raw": {**{y: f"data/raw/NQ_v0_1s_{y}.parquet"
            ...
            "states": "studies/regime_classification/results/states_nq_5m.parquet",
            "mult": 20.0},
    "ES": {"raw": {**{y: f"data/raw/ES_v0_1s_{y}.parquet"
            ...
            "states": "studies/regime_classification/results/states_es_5m.parquet",
            "mult": 50.0},
}
...
def lookup_state_causal(target_ts_arr, state_ts_arr, state_arr, bar_duration_ns):
    """
    Look up the state of the latest bar that has fully closed before or at target_ts_arr.
    A bar starting at T_open is fully closed at T_open + bar_duration_ns.
    So we look for the largest T_open <= target_ts_arr - bar_duration_ns.
    """
    ...
    # We want to find the largest index where state_ts_arr <= query_ts.
    # np.searchsorted(..., query_ts, side="right") returns the first index where state_ts > query_ts.
    # Subtracting 1 gives the largest index where state_ts <= query_ts.
...
def assess(cohort, state_col_values, state_ts_arr, state_moment,
            outcome_col, label):
    """Per-state pooled OOS lift, with per-OOS-year breakdown."""
    cohort = cohort.copy()
    if state_moment == "flip":
        target_ts = cohort["entry_ts"].to_numpy(np.int64)
    else:
        target_ts = (cohort["entry_ts"] + 60 * NS).to_numpy(np.int64)
        
    cohort["state"] = lookup_state_causal(target_ts, state_ts_arr, state_col_values, 300 * NS)
...
def main():
    ...
    # Compute +1/-1 ATR bracket outcome from bar1-close entry
    ...
    # ── HEADLINE: hmm_4 detail (5m macro) ──
    for headline in ("hmm_4",):
        print(f"\n{'='*88}\nHEADLINE {headline.upper()} — both state moments x both exits (5m macro)\n{'='*88}")
        state_arr = states[headline].to_numpy(np.int64)
        for state_moment in ("flip", "bar1"):
            for outcome_col, outcome_name in (
                ...
                if not rows:
                    continue
                print(f"\n  state_{state_moment} × {outcome_name}  "
                      f"(OOS base = {base:.1%})")
    ...
    # ── FULL SWEEP: all 12 models × 2 state moments × 2 exits (5m macro) ──
    ...
    cohort.to_parquet(OUT / f"bar5_deployable_state_{PRODUCT.lower()}.parquet")

"""Run the survivor / add-on forward-expectancy replay over a year.

    python studies/stairstep_exits/run_survivor.py --year 2021 [--smoke 7]
"""
...
from survivor_replay import SurvivorReplay  # noqa: E402
...
def main():
    ...
    args = ap.parse_args()
    year = args.year; OUT.mkdir(parents=True, exist_ok=True)
    ...
    print(f"  loaded {len(bars):,} bars ({time.time()-t0:.0f}s)")
    if not bars:
        return
    ...
    rep.finalize()
    print(f"  replay done ({time.time()-t0:.0f}s); trades={len(rep.records):,}")
    ...
    p = OUT / f"survivor_{year}{suffix}.parquet"
    df.to_parquet(p, index=False)
    print(f"  in-year trades={len(df):,} (A={int((df.population=='A').sum()):,} "
          f"B={int((df.population=='B').sum()):,}) -> {p}")

"""Bootstrap CI on no-delay V_A E (delayed @+7m + dual momentum).

Same battery as bootstrap_va_e.py and bootstrap_va_c.py — direct
comparison.
"""
...
def main():
    t0 = time.time()
    print("=" * 78)
    print("BOOTSTRAP CI on V_A E (NO-DELAY data)")
    print("=" * 78)
    ...
    is_alive = feats[feats["alive_7m"] & feats["year"].isin([2024, 2025])]
    thr_150 = is_alive["f_net_move_150s_7m"].quantile(0.10)
    thr_300 = is_alive["f_net_move_300s_7m"].quantile(0.20)
    print(f"\n  no-delay E filters (IS-fit):")

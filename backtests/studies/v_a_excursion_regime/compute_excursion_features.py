...
def load_year_trades(year: int):
    """Load trades + decision_ts (joined from snapshots) for a year."""
    base = Path(f"collectors/collector_v2/results/v_a_{year}")
    trades = pd.read_parquet(base / "trades.parquet")
    snaps = pd.read_parquet(base / "snapshots.parquet")
    snaps = snaps[snaps["became_trade"] == True][["event_id", "decision_ts"]]
    trades = trades.merge(
    ...
    if trades["decision_ts"].isna().any():
        ...
        print(f"  WARN: {n} trades missing decision_ts after join")
    return trades
...
def process_year(year: int):
    ...
    out_path = OUT / f"v_a_{year}_with_excursion.parquet"

"""Multi-year 1s-bar validation 2020-2026 — PT=1.0 / SL=2.0 variant.

Same architecture, data lineage, fill model, and dedup as
`validate_1s_multi_year.py`. Only PT_ATR and SL_ATR are changed.
Output goes to `multi_year_1s_summary_pt10.csv` /
`multi_year_1s_trades_pt10.csv` so neither file collides with the
PT=1.5/SL=3.0 results.
"""
...
def main():
    ...
    sdf.to_csv(OUT / "multi_year_1s_summary_pt10.csv", index=False)
    ...
    if len(all_df) > 0:
        ...
        all_df.to_csv(OUT / "multi_year_1s_trades_pt10.csv", index=False)

    print("\n" + "=" * 70)
    print(f"MULTI-YEAR 1s VALIDATION SUMMARY (PT={PT_ATR} / SL={SL_ATR} + F4)")
    print("=" * 70)
    ...
    print(f"\nWrote: {OUT/'multi_year_1s_summary_pt10.csv'}, "
          f"{OUT/'multi_year_1s_trades_pt10.csv'}")

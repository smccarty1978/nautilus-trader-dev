"""Tick-NT validation: PT=1.5 / SL=3.0 ATR with F4 filter applied.

Differences from tick_validate_eod_flatten.py:
  - PT_ATR = 1.5 (was 1.0)
  - SL_ATR = 3.0 (was 2.0)
  - F4 filter applied at signal time: trade only if total_exc_fast >= 57.75
    (computed inline from the 5-min lookback window ending at the signal bar,
    same logic as feature_scan_signals.py).

Fill mechanics (per memory rule feedback_real_fills_from_top_of_book.md):
  - ENTRY (market BUY): fill at ask_px_00 of first T row at-or-after signal_ts.
  - PT (resting limit at pt_px): trigger when trade price >= pt_px;
    fill at pt_px EXACTLY.
  - SL (stop-market sell): trigger when trade price <= sl_px;
    fill at bid_px_00 at trigger row.
  - EOD: at 15:00 CT, market sell at last bid in window.
  - Cost: $5 RT commission only (NQ). No artificial spread tax.

Output: studies/reversion_after_5_streak/results/tick_pt15_sl30_f4_{year}.csv
"""
...
EMA_LONG = 9
STREAK_LEN = 4
PT_ATR = 1.5                  # CHANGED from 1.0
SL_ATR = 3.0                  # CHANGED from 2.0
F4_CUTOFF = 57.75             # IS-derived Q5 boundary on total_exc_fast
FAST_WINDOW = 5               # 5-min lookback for F4 feature
NQ_MULT = 20.0
MNQ_MULT = 2.0
...
def compute_total_exc_fast(open_arr, high_arr, low_arr, close_arr, ts_arr,
                              window=FAST_WINDOW):
    """Causal rolling 5m total excursion (mfe + mae) at each bar.
    Window: bars [i-N+1 .. i] inclusive (includes the signal bar).
    NaN when window spans a 60s-gap (e.g., the 16:00-17:00 CT pause)."""
    n = len(open_arr)
    ...
    for i in range(window - 1, n):
        ...
        mfe = w_high - start_open
        mae = start_open - w_low
        out[i] = mfe + mae
    return out
...
def find_signals(df, atr, regime, tef, roll_dates, roll_filter_days):
    """Generate signals with F4 filter applied. F4 = total_exc_fast >= F4_CUTOFF
    at the signal bar."""
    ...
    for i in range(STREAK_LEN - 1, len(df)):
        ...
        if eod_ts - sig_ts < 60_000_000_000:
            continue
        # F4 filter (causal, computed at signal bar)
        f4_val = tef[i]
        if not np.isfinite(f4_val):
            continue
        n_pre_f4 += 1
        if f4_val < F4_CUTOFF:
            continue
    ...
    return out, n_pre_f4, n_pre_roll
...
def load_trade_rows_with_quotes(tick_files, win_lo_ns, win_hi_ns):
    ...
    for path in tick_files:
        ...
        try:
            ...
            print(f"  filter-read failed: {e}, falling back", flush=True)
        ...
        parts.append(df)
        print(f"  {path}: {len(df):,} T rows ({time.time()-t0:.0f}s)", flush=True)
    if not parts:
        return (np.array([], dtype=np.int64),) * 4
    big = pd.concat(parts, ignore_index=True)
    big = big.sort_values("ts_event", kind="stable").reset_index(drop=True)
    return (big["ts_event"].astype("int64").to_numpy(),
            big["price"].astype(np.float64).to_numpy(),
            big["bid_px_00"].astype(np.float64).to_numpy(),
            big["ask_px_00"].astype(np.float64).to_numpy())
...
def replay_signal(signal, ts_arr, px_arr, bid_arr, ask_arr):
    ...
    if n_ticks > 0:
        hit_pt = seg_px >= pt_px      # trade price reached limit
        hit_sl = seg_px <= sl_px      # trade price hit stop
        if hit_pt.any() or hit_sl.any():
            ...
            if i_sl <= i_pt:
                ...
                exit_ts = int(seg_ts[idx])
                exit_fill = float(pt_px)     # limit fills at limit
        else:
...
def run_one_year(year):
    ...
    print("\n" + "=" * 70, flush=True)
    print(f"YEAR {year}  |  PT={PT_ATR} SL={SL_ATR}  |  F4 cutoff = {F4_CUTOFF}",
          flush=True)
    ...
    signals, n_pre_f4, n_pre_roll = find_signals(
        df, atr, regime, tef, cfg["roll_dates"], cfg["roll_filter_days"])
    print(f"  pre-F4 candidates : {n_pre_f4:,}", flush=True)
    print(f"  post-F4, pre-roll : {n_pre_roll:,}", flush=True)
    print(f"  post-everything   : {len(signals):,}", flush=True)
    if not signals:
    ...
    print(f"  single-position replay: {len(rows):,} taken, "
          f"{n_overlap:,} skipped ({time.time()-t0:.0f}s)", flush=True)
    ...
    out.to_csv(OUT / f"tick_pt15_sl30_f4_{year}.csv", index=False)
    ...
    valid["peak_nq"] = valid["cum_nq"].cummax()
    valid["dd_nq"] = valid["cum_nq"] - valid["peak_nq"]
    valid["cum_mnq"] = valid["net_pnl_mnq"].cumsum()
    valid["peak_mnq"] = valid["cum_mnq"].cummax()
    valid["dd_mnq"] = valid["cum_mnq"] - valid["peak_mnq"]
    ...
    print(f"\n--- {year} RESULT (PT={PT_ATR} SL={SL_ATR} F4-filtered tick) ---")
    print(f"  trades            : {n_v:,}")
    print(f"  PT / SL / EOD     : {pt} / {sl} / {eod}")
    print(f"  WR resolved       : {wr:.2f}%")
    print(f"  WR full (NET>0)   : {(valid['net_pnl_nq']>0).sum()/n_v*100:.2f}%")
    print()
    print("  NQ (per contract, $5 RT commission):")
    print(f"    Mean $/trade gross : {valid['gross_pnl_nq'].mean():+.2f}")
    print(f"    Mean $/trade NET   : {valid['net_pnl_nq'].mean():+.2f}")
    print(f"    Total NET          : {valid['net_pnl_nq'].sum():+,.0f}")
    print(f"    Max DD             : {valid['dd_nq'].min():+,.0f}")
    print(f"    Worst single trade : {valid['net_pnl_nq'].min():+,.2f}")
    print(f"    Best single trade  : {valid['net_pnl_nq'].max():+,.2f}")
    print()
    print("  MNQ (per contract, $2.50 RT commission):")
    print(f"    Mean $/trade gross : {valid['gross_pnl_mnq'].mean():+.2f}")
    print(f"    Mean $/trade NET   : {valid['net_pnl_mnq'].mean():+.2f}")
    print(f"    Total NET          : {valid['net_pnl_mnq'].sum():+,.0f}")
    print(f"    Max DD             : {valid['dd_mnq'].min():+,.0f}")
    print(f"    Worst single trade : {valid['net_pnl_mnq'].min():+,.2f}")
    print(f"    Best single trade  : {valid['net_pnl_mnq'].max():+,.2f}")
    print()
    print(f"  Hold time (s)      : mean={valid['hold_seconds'].mean():.0f}  "
          f"median={valid['hold_seconds'].median():.0f}  "
          f"p90={valid['hold_seconds'].quantile(0.9):.0f}")
    print(f"  Spread @ entry (pts): mean={valid['spread_at_entry'].mean():.3f}  "
          f"median={valid['spread_at_entry'].median():.3f}")
    print(f"  Bad-quote fallbacks: entry_patched={valid['entry_ask_patched'].sum()}  "
          f"exit_patched={valid['exit_bid_patched'].sum()}")
...
def main():
    ...
    for year in [2026, 2025]:
        run_one_year(year)
    print("\nWrote: tick_pt15_sl30_f4_{2025,2026}.csv")

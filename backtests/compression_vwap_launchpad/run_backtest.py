...
from strategy import CompressionLaunchpadLongConfig, CompressionLaunchpadLongStrategy
...
def main():
    ...
    ap.add_argument("--excursion-cap", type=float, default=22.0)
    ap.add_argument("--excursion-window", type=int, default=30)
    ap.add_argument("--out-tag", default=None,
                    help="optional subdir tag for sweep runs")
    args = ap.parse_args()

    year = args.year
    if args.out_tag:
        out_dir = Path(f"backtests/compression_vwap_launchpad/results/"
                       f"sweep/{args.out_tag}/live_{year}")
    else:
        out_dir = Path(f"backtests/compression_vwap_launchpad/results/live_{year}")
    ...
    engine.add_venue(
        ...
        bar_execution=True,
        bar_adaptive_high_low_ordering=True,  # heuristic intra-bar OHLC order
    )
    engine.add_instrument(nq)
    engine.add_data(bars_1s)

    strat_cfg = CompressionLaunchpadLongConfig(
        instrument_id="NQ.XCME",
        position_size=2,  # 2-lot runner
        excursion_cap=args.excursion_cap,
        excursion_window=args.excursion_window,
    )
    strat = CompressionLaunchpadLongStrategy(strat_cfg)
    ...
    if len(trades) > 0:
        # Filter to in-year entries only
        ...
        in_year = trades[
            ...
            (trades["entry_ts"] <= yr_end_ns)
        ].copy()
        print(f"In-year trade records: {len(in_year)}")

        # Exit reason breakdown
        if len(in_year):
            print("\n--- Exit Reason Breakdown ---")
            print(in_year["exit_reason"].value_counts().to_string())
            print(f"\n--- T1 Fill Rate ---")
            print(f"  T1 filled: {in_year['t1_filled'].sum()} / {len(in_year)} "
                  f"({in_year['t1_filled'].mean():.1%})")
    ...
    # Account balance (net engine PnL)
    report = engine.trader.generate_account_report(Venue("XCME"))
    print("\n--- Account Report ---")

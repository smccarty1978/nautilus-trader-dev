"""Run V_A reference through Collector V2 against the NQ.v.0 multi-year
catalog. Same logic as run_v_a_year.py but points at the new catalog
(volume-continuous) so excursion analysis is contract-consistent.

Output: collectors/collector_v2/results/v_a_v0_<year>/
"""
...
def main():
    ...
    out_dir = Path(
        f"collectors/collector_v2/results/v_a_v0_{args.year}")
    ...
    print(f"\nRunning V_A Mode 2 for {args.year} on NQ.v.0...", flush=True)
    t0 = time.time()

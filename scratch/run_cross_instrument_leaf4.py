...
INSTRUMENTS = {
    'ES': {
        ...
        'commission_rt': 5.0,
        'slippage_rt_ticks': 2,
        'total_friction_pts': 0.50 + 0.10, # 2 ticks (0.50) + $5 comm (0.10 pts) = 0.60 pts ($30)
        'total_friction_dollars': 30.0,
    },
    'YM': {
        'point_value': 5.0,
        'tick_size': 1.0,
...
def process_instrument(inst_name):
    ...
    print(f"\n=======================================================")
    print(f"PROCESSING INSTRUMENT: {inst_name}")
    print(f"=======================================================")
    ...
    for year in YEARS:
        ...
        # Precompute 1m features and regimes
        ...
        # Convert index to Chicago time for RTH session filtering
        ...
        # RTH mask: 08:30 (510 min) to 15:15 (915 min)
        ...
        # Now detect H050 checkpoints during RTH across regimes
        print("Detecting H050 pullback checkpoints during RTH...")
        t_ck = time.time()
        
        # Filter regimes that intersect RTH
        reg_df = pd.DataFrame(regimes)
        
        # For fast lookup, iterate through each regime
        year_ck_list = []
        for _, reg in reg_df.iterrows():
            ...
            # Causal state machine within regime
            ...
            for k in range(len(r_ts)):
                ...
                if d == 1:
                    if h_k > running_mfe_px:
                        running_mfe_px = h_k
                        in_pullback = False # Reextension
                else:
                    if l_k < running_mfe_px:
                        running_mfe_px = l_k
                        in_pullback = False # Reextension
                        
                # Check pullback from running MFE
                if d == 1:
                ...
                if (not in_pullback) and (pb_depth_atr >= 0.50):
                    # Qualifying H050 checkpoint!
                    in_pullback = True
                    
                    # Only emit if within RTH
                    if r_rth[k]:
                        min_from_rth_open = float(r_ct_m[k] - 510 + r_ct_s[k] / 60.0)
                        
                        # 1m lookup for realized range and atr
        ...
        # Now simulate counter-regime trade outcomes for Leaf 4 and all checkpoints
        print("Simulating counter-regime C1 trade outcomes...")
        t_trade = time.time()
        
        # Sort checkpoints by ts
        year_ck_list.sort(key=lambda x: x['checkpoint_ts'])
        
        for idx_ck, ck_item in enumerate(year_ck_list):
            pos_1s = ck_item['1s_pos']
            ck_ts = ck_item['checkpoint_ts']
            reg_exit = ck_item['regime_exit_ts']
            d = ck_item['direction']
            c_dir = -d # Counter-regime trade direction
            atr = ck_item['frozen_atr']
            
            # Entry at next 1s bar open
            ...
            # Exit at next opposing H050 checkpoint or regime exit
            # Opposing H050 means next checkpoint with direction == -d
            opposing_ts = None
            for next_ck in year_ck_list[idx_ck+1:]:
    ...
    # Compute metrics for Leaf 4 across epochs
    def compute_leaf4_metrics(df_sub, n_days, total_pop):
        ...
        df_sub['month'] = df_sub['dt'].dt.to_period('M').astype(str)
...
df_es = process_instrument('ES')
df_ym = process_instrument('YM')
print("\nCross-instrument processing finished successfully.")

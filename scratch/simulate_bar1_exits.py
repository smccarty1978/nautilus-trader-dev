...
def simulate_single_trade_path(idx_start, idx_end, px_entry, atr, d, ts_start, ts_1s, high_1s, low_1s, close_1s,
    ...
    """Trace a single trade path chronologically with PT, SL, and a Time/PnL Gate."""
    pt_px = px_entry + d * pt_atr * atr
    sl_px = px_entry - d * sl_atr * atr
    
    # Round target and stop to nearest tick for parity with execution
    pt_px_rounded = round(pt_px * 4) / 4
    sl_px_rounded = round(sl_px * 4) / 4
    
    # Search loop
    exit_px = np.nan
    exit_reason = 0 # 1=PT, 2=SL, 3=Gate, 4=Flip
    ...
    # If we reached the end of the trade (regime flip) without hitting SL/PT/Gate:
    if np.isnan(exit_px):
...
def main():
    ...
    print(f"Loaded {len(df_bar1):,} bar1-confirmed trades for RTH/ETH 1s simulation sweep.")
    ...
    print("\n" + "="*145)
    print("  SIMULATION SWEEP: GROSS VS NET ANALYSIS FOR BAR1-CONFIRMED REGIME FLIPS (2020-2026)")
    print("="*145)
    ...
    for pt, sl, gt, g_thresh, g_label in combinations:
        df_sim = run_simulation_sweep(df_bar1, pt, sl, gt, g_thresh)
    ...
    print("\n" + "="*95)
    print(f"  YEARLY BREAKDOWN FOR BEST SETUP (NET): PT={best['pt']} / SL={best['sl']} / {best['label']}")
    print("="*95)

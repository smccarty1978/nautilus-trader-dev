"""Calculate MFE distribution for regimes lasting 15 minutes or longer."""
import os, sys, time
import numpy as np
import pandas as pd
...
def run_analysis():
    ...
    df_long = df_flips[df_flips["hold_min_flip"] >= 15.0].copy()
    print(f"Total long regimes (>= 15 min): {len(df_long):,} (out of {len(df_flips):,} total flips)")
    ...
    for y in years:
        df_y = df_long[df_long["year"] == y].copy().reset_index(drop=True)
        if len(df_y) == 0:
            continue
            
        print(f"Processing year {y} ({len(df_y)} long episodes)...")
    ...
    print("\n" + "="*60)
    print("  MFE DISTRIBUTION FOR LONG REGIMES (>= 15 MIN)")
    print("="*60)

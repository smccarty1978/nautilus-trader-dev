"""Load and analyze raw flips ablation vs production results with correct min_atr suffixes."""
import pandas as pd
import numpy as np
...
def main():
    print("ANALYZING RAW FLIP PRODUCTION (A) vs. RAW FLIP ABLATION (B) (vol > 15)...")
    
    # 1. Analyze Production Baseline (hmm_4 state 3, flip entry, min_atr 15.0)
    prod_y, prod_p = analyze_sweep("nq_hmm_4_s3_pt2p0_ancflip_flip_p4_minatr15p0")
    
    # 2. Analyze Ablation (no state filter, flip entry, min_atr 15.0)
    ab_y, ab_p = analyze_sweep("nq_hmm_4_s-1_pt2p0_ancflip_flip_ablation_minatr15p0")
        
    if prod_p is None or ab_p is None:
        print("Error: Could not load one of the sweeps.")
        return
        
    print("\n--- SIDE-BY-SIDE RAW FLIPS COMPARISON TABLE ---")

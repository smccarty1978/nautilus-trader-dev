"""Determine how many of the bad entries lose money when held strictly to opposite flip (no stop-loss).

Traded with:
- NO STOP-LOSS (just hold until the opposite regime flip)
- Entry at Bar 2 Open (for Bar 2 filters)
- Entry at Bar 4 Open (for Bar 4 Version B filter)
"""
import numpy as np
import pandas as pd
...
def main():
    ...
    # Helper function to trade with NO STOP LOSS
    def get_trade_pnl_no_stop(row, entry_bar):
        ...
        # Exits strictly at the opposite flip close (the last bar of the regime)
        exit_px = post_c[-1] - d_val * EXIT_SLIP
        return (exit_px - entry_fill) * d_val * MULT - COMM

    # --- PART 1: BAR 2 ENTRIES ---
    print("=== BAR 2 ENTRY FILTERS (NO STOP-LOSS, OOS 2025-2026) ===")
    ...
    for fname, mask in bar2_filters:
        ...
        sub_indices = idx_oos[bad_mask]
        sub_df = df.iloc[sub_indices]
        
        total_bad = len(sub_df)
        if total_bad == 0:
            print(f"\n{fname}: No bad entries.")
            continue
            
        pnls = [get_trade_pnl_no_stop(row, 2) for _, row in sub_df.iterrows()]
        p_arr = np.array(pnls)
        ...
        print(f"\n{fname}:")
        print(f"  Bad Entries:               {total_bad:,}")
        print(f"  Fail to be Profitable (pnl < 0): {unprof:,} ({unprof/total_bad*100:.1f}%)")
        print(f"  Ended up Profitable (pnl > 0):   {prof:,} ({prof/total_bad*100:.1f}%)")
        if flat > 0:
            print(f"  Ended Flat (pnl == 0):           {flat:,} ({flat/total_bad*100:.1f}%)")

    # --- PART 2: BAR 4 ENTRY (VERSION B) ---
    print("\n=== BAR 4 ENTRY FILTER (VERSION B) (NO STOP-LOSS, OOS 2025-2026) ===")
    sub_df_b = df[verB_mask & (df["is_tradable"] == 0)]
    total_bad_b = len(sub_df_b)
    
    if total_bad_b > 0:
        pnls_b = [get_trade_pnl_no_stop(row, 4) for _, row in sub_df_b.iterrows()]
        p_arr_b = np.array(pnls_b)
        
        unprof_b = (p_arr_b < 0).sum()
        prof_b = (p_arr_b > 0).sum()
        flat_b = (p_arr_b == 0).sum()
        
        print(f"Version B Filter (Bad Entries entered at Bar 4 Open):")
        print(f"  Bad Entries:               {total_bad_b:,}")
        print(f"  Fail to be Profitable (pnl < 0): {unprof_b:,} ({unprof_b/total_bad_b*100:.1f}%)")
        print(f"  Ended up Profitable (pnl > 0):   {prof_b:,} ({prof_b/total_bad_b*100:.1f}%)")
        if flat_b > 0:
            print(f"  Ended Flat (pnl == 0):           {flat_b:,} ({flat_b/total_bad_b*100:.1f}%)")

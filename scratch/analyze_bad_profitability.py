"""Determine how many of the bad entries actually lose money when held to the opposite regime flip.

Traded with:
- 1.0 ATR Stop-loss
- Macro Exit (Opposite Regime Flip)
...
def main():
    ...
    def get_trade_pnl(row, entry_bar):
        d_val = row["direction"]
    ...
    for fname, mask in bar2_filters:
        # Get bad entries (entered AND is_tradable == 0)
        ...
        pnls = [get_trade_pnl(row, 2) for _, row in sub_df.iterrows()]
        p_arr = np.array(pnls)
    ...
    if total_bad_b > 0:
        pnls_b = [get_trade_pnl(row, 4) for _, row in sub_df_b.iterrows()]

"""First-passage oracle parity for one-sided milestone barrier arms.

Relocated out of studies/nq_va_multitf_trade_state_geometry/fixtures/ because R10
(zero_study_python) forbids committed Python under a study tree. It is a proof about the
PLATFORM kernel, so it belongs here; the study keeps the JSON evidence card it emits.
...
CARD_PATH = pathlib.Path(__file__).with_name("first_passage_parity.json")


def test_first_passage_oracle_parity_and_terminal_preserved():
    assert main() == 0, "see the emitted card for the failing cases"

"""Run pullback NT runtime strategy on a year."""
...
from pullback_strategy import (  # noqa: E402
...
def main():
    ...
    out_dir = Path(args.out_dir or
                     f"studies/pullback_entry_v1/results/"
                     f"nt_runtime_{args.year}")
    out_dir.mkdir(parents=True, exist_ok=True)

    load_start = pd.Timestamp(f"{args.year}-01-01", tz="UTC") \
                    - pd.Timedelta(days=5)
    load_end = pd.Timestamp(f"{args.year}-12-31 23:59:59", tz="UTC")

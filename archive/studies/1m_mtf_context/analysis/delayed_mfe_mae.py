"""Forward MFE/MAE from each delayed entry point.

For each delay D in {30s, 60s, 90s, 120s, 180s, 240s, 300s}:
  - Filter to surviving trades
  - New entry = close at +D after current entry
  - Walk remaining trade window, compute forward peak MFE and MAE
    from the NEW entry price as baseline
  - Report percentile distributions
"""
...
def forward_mfe_mae(entry_ts, exit_ts, direction, atr,
    ...
    """Find new entry at delay, walk forward, compute peak MFE/MAE.

    Returns (survived, new_entry_px, peak_mfe, peak_mae)
    Returns (0, 0, 0, 0) if trade ended at or before delay.
    """
...
def main():
    ...
    # Baseline (delay=0 = full trade, current entry)
    print(f"\n{'='*100}")
    print(f"BASELINE (current entry, no delay) — full-trade MFE/MAE")
    print(f"{'='*100}")
    print(percentiles("Peak MFE (ATR)", trades["peak_mfe_atr"].values))
    print(percentiles("Peak MAE (ATR)", trades["peak_mae_atr"].values))

    print(f"\n{'='*100}")
    print(f"FORWARD MFE/MAE FROM DELAYED ENTRY (only surviving trades)")
    print(f"{'='*100}")
    ...
    for delay_s in DELAYS_S:
        ...
        sub_mfe = peak_mfe[surv_mask]
        sub_mae = peak_mae[surv_mask]

        print(f"\n--- Delay +{delay_s}s ({n_surv:,} survived) ---")
        print(percentiles("Forward peak MFE", sub_mfe))
        print(percentiles("Forward peak MAE", sub_mae))

        summary.append({
            "delay_s": delay_s, "n_surv": n_surv,
            ...
            "mfe_mae_ratio": sub_mfe.mean() / sub_mae.mean(),
        })

    # Compact summary
    print(f"\n{'='*100}")
    print(f"COMPACT SUMMARY — Forward MFE/MAE from delayed entry")
    print(f"{'='*100}")
    print(f"  {'Delay':>6} {'N':>7} | "
    ...
    print("  " + "-" * 105)
    # Baseline first
    base_mfe = trades["peak_mfe_atr"].values
    base_mae = trades["peak_mae_atr"].values
    print(f"  {'NONE':>6} {len(base_mfe):>7,} | "
    ...
    for s in summary:
        print(f"  {s['delay_s']:>5}s {s['n_surv']:>7,} | "

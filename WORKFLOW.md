...
| Capability registry (seeds) | Hand-maintained seed entries; a kind with its own registration boundary keeps its own file (§21.14) | `research_workflow/capabilities_index.yaml`, `research_workflow/capabilities_index.d/<kind>.yaml`, `research_workflow/capabilities.py` | via `cap propose/scaffold/promote` | NO | `research cap list/search/describe` |
| Registration boundaries | Resolution API + index-driven discovery, so adding a capability cannot change how an existing one resolves | `features/trackers/host_bindings.py` (trackers), `features/registry.py` (feature definitions, catalogues in `features/definitions/`), `research/analysis/ops.py` (analysis ops) | NO (platform change) | NO | see `docs/RESEARCH_WORKFLOW.md` §21.14 |
...
| Feature metadata / definitions | Canonical identities | `features/definitions/*.py` (catalogues), `features/feature_types.py` (`FeatureDefinition`), `features/registry.py` (resolver), `features/CANONICAL_FEATURE_REFERENCE.yaml` | NO | YAML yes (`scripts/generate_canonical_feature_reference.py`) | `research cap describe <feature>` |
...
| Analysis operations | Declarative post-collection statistics | `research/analysis/diagnostic_ops.py` (implementations), `research/analysis/ops.py` (resolution boundary) | via capability flow | NO | `research cap list analysis_ops` |

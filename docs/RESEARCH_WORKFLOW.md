...
frame. All operations are study-agnostic — the science lives in the declared parameters:
...
| `analysis.derive.columns` | declared **row-wise** columns in a bounded grammar that is parsed, never `eval`-ed (`research/analysis/expressions.py`): `+ - * /`, comparisons, three-valued `and/or/not`, and the closed function set `abs sign sqrt min max is_null not_null where coalesce concat cut`. NULL propagates; `a / 0` is NULL (`div_zero: null`, default) or refused (`error`); integer columns (ns timestamps) stay exact. `each:` templates expand a column over declared values; `keep:` filters rows (dropped rows counted). No aggregation, no cross-row access, never overwrites a column. **The compiler refuses any expression referencing a column the plan does not provide** (identity + metadata + features + observation + `_year`, or a column derived earlier; the outputs of `describe.grouped` / `clustered_mean` / `classify.precedence` are provable, other ops' are not). |
| `analysis.contrast.nominate` | frozen nomination rules over parent/child contrasts: for each declared parent (`parent_by`, or several `contrasts: [{parent_by, dimensions}]` forming **one** family), each child value and metric, child vs rest-of-parent with the `clustered_mean` difference estimator (CR1, session-day clusters); declared support gates (`min_child_n`, `min_parent_n`, `min_nominate_n`); Benjamini-Hochberg over the whole tested family; declared materiality (absolute and relative child-vs-parent difference, CI excludes 0, `max_bh_q`). Every contrast is emitted in key order (never ranked); the JSON payload is the **claim file** — specification, gates, replication rules and claims frozen together. No number is hard-coded. |
| `analysis.replication.scorecard` | re-measures **every** claim of a committed claim file whose sha256 is pinned in the step (a changed file refuses to run) with the file's own contrast specification, and classifies each by the file's own ordered rules (`[{class, requires: [predicates]}]`, first match; predicates `underpowered estimate_null same_sign opposite_sign ratio_ge_min ratio_lt_min ci_excludes_zero ci_includes_zero`; unmatched → fallback class, retained). Cannot re-threshold, drop or add a claim. |

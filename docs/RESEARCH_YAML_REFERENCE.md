...
| `streams` | list[object (StreamSpec)] | yes |  |  | Datasets and timeframes; the first is the execution stream. | Only external timeframes declared in the DatasetSpec are read from disk; the rest are host-derived complete buckets. | `streams: [{dataset: NQ_1S_V2_GLOBEX, timeframes: [1s, 1m]}]` |
| `streams[].dataset` | str | yes |  |  | Committed DatasetSpec id (research/datasets/<id>.yaml); never a path. | The plan binds the dataset logical digest; readiness verifies bytes. | `dataset: NQ_1S_V2_GLOBEX` |
...
| `outcome.session_end` | enum | no | censor | 'censor', 'ignore', 'truncate' | censor: an arm/flip whose horizon passes the session close is CENSORED SESSION_END whether or not the event happened; truncate: the session close bounds the observation window instead of voiding it -- an event at or before the close resolves POSITIVE at its own instant, and only a candidate that reached the close without one is CENSORED SESSION_END, stamped at the close; ignore: no session boundary. | truncate is how 'observe until the session ends' is expressed: under censor a horizon longer than the session censors every candidate, so time-to-event past a fixed horizon cannot be measured at all. | `session_end: truncate` |
...
| `chronology.windows` | list[str] | no | list() |  | Date-bounded partition execution: 'YYYY-MM-DD..YYYY-MM-DD' (inclusive), one calendar year per window. | A window NARROWS an already-authorized train/dev year to explicit dates -- it never opens a year the roles did not authorize, and a year that carries windows streams and emits ONLY those dates. The window is a hard data boundary with no forward lookahead tail: a candidate whose outcome cannot resolve inside it fails the partition (WINDOW_OUTCOME_UNRESOLVED) rather than censoring silently. | `windows: ['2024-03-01..2024-03-31']` |
...
| `model.arms` | list[str | object (ArmSpec)] | no | list() |  | Predeclared feature architectures, each fit independently on the same collected population. | A BARE list of names is refused in train mode (SEMANTIC_DECISION_REQUIRED): it used to compile and silently train ONE model on the whole surface. Declare {id, features, baseline}. Exactly one arm must be baseline: true. | `arms: [{id: A, baseline: true, features: [x, y]}, {id: B, features: [x, y, z]}]` |
| `model.arms[].id` | str | yes |  |  | Arm name; appears in the model lineage as target_arm. | Unique across arms. | `id: BASELINE` |
| `model.arms[].features` | list[str] | no | list() |  | This arm's own subset of the declared feature surface. | Every entry must be a declared feature or derived column; empty means the full surface. | `features: [arrival_velocity, ema_slope]` |
| `model.arms[].params` | map[str -> Any] | no | dict() |  | Arm-level hyperparameter override. | Merged over model.params and the cell's params. | `params: {n_estimators: 200}` |
| `model.arms[].baseline` | bool | no | False |  | Marks the control arm every other arm is compared against. | Exactly one arm must set it; paired deltas are computed against it. | `baseline: true` |
| `model.cells` | list[object (CellSpec)] | no | list() |  | Population cells fit independently, each with its own fixed hyperparameters (typically a direction). | Cells partition by an explicit column == value filter; there are no hidden direction semantics. | `cells: [{id: LONG, subset: {regime_direction: 1}, params: {n_estimators: 100}}]` |
| `model.cells[].id` | str | yes |  |  | Cell name; appears in the model lineage as cell_id. | Unique across cells. | `id: LONG` |
| `model.cells[].subset` | map[str -> Any] | no | dict() |  | Explicit column == value row filter selecting this cell's rows. | Every column must be one the study emits. | `subset: {regime_direction: 1}` |
| `model.cells[].params` | map[str -> Any] | no | dict() |  | This cell's fixed hyperparameters. | Merged over model.params; an arm's params win over a cell's. | `params: {learning_rate: 0.0394, n_estimators: 100}` |
...
| `model.validation.fold_months_year` | int | null | no | None |  | The single calendar year rolling monthly folds walk across. | Must be a declared TRAIN year; a dev or prohibited year is refused. | `fold_months_year: 2024` |
| `model.validation.train_months` | int | null | no | None |  | Width in months of each fold's fit window. | walk_forward_months only. | `train_months: 3` |
| `model.validation.validate_months` | int | null | no | None |  | Width in months of each fold's validation window. | walk_forward_months only; always strictly after the fit window. | `validate_months: 1` |
| `model.validation.step_months` | int | no | 1 |  | How far the window advances between folds. | walk_forward_months only. | `step_months: 1` |
...
| `model.models[].expect.canonical_sha256` | str | null | no | None |  | Expected manifest canonical.byte_sha256 -- binds to the estimator's actual bytes, not only lineage; catches a substituted estimator that refreshes its own canonical/golden bytes under an unchanged model_id. | A wrong or stale declared value fails closed as CANONICAL_SHA_MISMATCH before score(). | `canonical_sha256: 3b1c...a2` |
| `model.reference_models` | list[object (ScoredModelSpec)] | no | list() |  | Frozen models scored ALONGSIDE the arms this study trains, on the identical population. | Never refit. mode stays train. Without this a descriptive reference needs its own study, and then it is not the same rows. | `reference_models: [{id: <sha256>, name: PARENT_LONG, label: target_flip_within_horizon, subset: {regime_direction: 1}}]` |
| `model.reference_models[].id` | str | yes |  |  | Model-store id of the frozen model. | 64 hex characters. | `id: ccd587df...` |
| `model.reference_models[].label` | str | yes |  |  | Outcome column the reference is evaluated against. | Must be an outcome column of this study. | `label: target_flip_within_horizon` |
| `model.reference_models[].subset` | map[str -> Any] | no | dict() |  | Explicit column == value row filter for this reference. | None. | `subset: {regime_direction: 1}` |
| `model.reference_models[].name` | str | null | no | None |  | Reporting name for the reference. | Defaults to the id prefix. | `name: PARENT_LONG_2021_2023` |
| `model.reference_models[].expect` | object (ScoredModelExpectSpec) | null | no | None |  | Identity expectations authenticated against the model-store lineage before scoring. | A mismatch refuses the scoring. | `expect: {study_id: parent_study}` |
| `model.reference_models[].expect.study_id` | str | null | no | None |  | Expected originating study id. | None. | `study_id: clean_maturity_flip_model_180s_horizon` |
| `model.reference_models[].expect.target_arm` | str | null | no | None |  | Expected target arm on the frozen lineage. | None. | `target_arm: C` |
| `model.reference_models[].expect.direction` | str | null | no | None |  | Expected direction on the frozen lineage. | None. | `direction: LONG` |
| `model.reference_models[].expect.cell_id` | str | null | no | None |  | Expected cell id on the frozen lineage. | None. | `cell_id: LONG_C` |
| `model.reference_models[].expect.canonical_sha256` | str | null | no | None |  | Expected canonical estimator BYTES. | Catches a substituted estimator that refreshed its own bytes under an unchanged model_id. | `canonical_sha256: f969037b...` |
...
| `analysis` | object (AnalysisSpec) | null | no | None |  | Declarative post-collection analysis: registered operations composed over the study's own collected frame. | Runs AFTER collection and may read outcome columns; it is never a feature surface. Its declared source decides whether the protected-OOS gate applies. | `analysis: {source: train, steps: [...], artifacts: [...]}` |
| `analysis.source` | enum | no | train | 'train', 'oos' | Which collected partition set the analysis frame comes from: 'train' or 'oos'. | source: oos opens the protected period through assert_oos_open and needs chronology.dev years; source: train never opens one. | `source: train` |
| `analysis.steps` | list[object (AnalysisStepSpec)] | no | list() |  | Ordered pipeline of registered analysis operations (`research cap list analysis_ops`). | Steps run in declaration order and may only read the study frame or an EARLIER step, so the pipeline is a DAG proven at compile time. | `steps: [{id: anchors, op: analysis.anchor.first_threshold_crossing, params: {...}}]` |
| `analysis.steps[].id` | str | yes |  |  | Step name; later steps and artifacts refer to it. | None. | `id: anchors` |
| `analysis.steps[].op` | str | yes |  |  | Registered analysis_ops capability id. | None. | `op: analysis.incidence.cumulative` |
| `analysis.steps[].gate` | bool | no | False |  | Run this step as a PRE-FIT gate at the population_parity stage instead of at analyze. | A gate runs on the reconciled candidate population BEFORE merge/fit, so a failure means no model is ever fit from that population. It must read 'frame' and can have no step inputs. | `gate: true` |
| `analysis.steps[].rows` | str | no | frame |  | Primary frame input: 'frame' (the collected study frame) or an earlier step id. | A forward reference is UNSUPPORTED_COMPOSITION. | `rows: anchors` |
| `analysis.steps[].inputs` | map[str -> str] | no | dict() |  | Extra frame inputs the operation declares, mapped to 'frame' or an earlier step id. | Missing or unknown inputs are typed gaps at compile time. | `inputs: {anchors: anchors}` |
| `analysis.steps[].params` | map[str -> Any] | no | dict() |  | The operation's declared parameters -- the science lives here, never in code. | None. | `params: {horizons: [60, 180, 300]}` |
| `analysis.artifacts` | list[object (AnalysisArtifactSpec)] | no | list() |  | The files this analysis produces under studies/<id>/artifacts. | An analysis that declares no artifact is refused: it could not be audited. | `artifacts: [{name: incidence.json, source: incidence}]` |

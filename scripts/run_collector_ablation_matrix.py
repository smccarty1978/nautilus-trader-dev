...
def run_one(name: str, root: Path) -> dict:
    ...
    try:
        ...
        return {"configuration": name, "wall_seconds": elapsed,
                "replay_seconds": timing.get("replay_seconds"),
                "setup_seconds": (elapsed - timing.get("replay_seconds", elapsed)),
                "bars_per_second": 207911 / elapsed, "candidates": candidates}
    finally:
...
def main() -> int:
    ...
    report = {name: run_one(name, root) for name in CONTROLS}

...
def render() -> str:
    ...
    lines += ["", "## Notes", "",
              ...
              "* DatasetSpec `rules.outage_gap_seconds` (dataset YAML, not the study grammar): the shortest run of missing native seconds that counts as a DATA OUTAGE rather than a quiet market. A `sessions` calendar switches the closed-window zero-volume fill on; the fill scope is then the trading day minus the declared maintenance halts and minus every `gaps` run at or above this threshold, and the suppressed windows are reported per stream as `windows_suppressed` (`halt` / `outage`) beside `empty_windows_published`. It is required (with a `gaps` reference table) on any calendar dataset a study derives a timeframe from -- without it the compile stops with `SEMANTIC_DECISION_REQUIRED`, because every empty window lies inside SOME gap run when the tape carries native rows only, so the runtime cannot derive the distinction. NQ/ES V2 declare 7200 s: the quiet-market maximum in an outage-free year is 299 s.", ""]
    text = "\n".join(lines)
    if missing:

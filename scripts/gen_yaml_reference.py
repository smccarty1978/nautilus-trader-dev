...
MEANING: Dict[str, Tuple[str, str, str]] = {
    ...
    "stage": ("'research' (default: the full claim lifecycle) or 'collect' (THE FOUR STAGES: produce an immutable frame).",
              "A collect study runs the controller through merge under a FRAME SEAL (closure composite + causal audit; the contract audit is NOT_REQUIRED because a frame makes no claim), fits no model, declares no dev years and no analysis, and -- for an every_candidate population -- must be PERMISSIVE (no population.qualify; carry the fields a later selection reads as features.metadata columns). It is registered with `research frame register --study <id>` under a content-derived frame id that is independent of the study id; it is never closed. The claim stages (fit, freeze, oos, analyze, close) refuse it with COLLECT_STAGE_NOT_APPLICABLE.",
              "stage: collect"),
    ...
    "outcome.session_end": ("censor: an arm/flip whose horizon passes the session close is CENSORED SESSION_END whether or not the event happened; truncate: the session close bounds the observation window instead of voiding it -- an event at or before the close resolves POSITIVE at its own instant, and only a candidate that reached the close without one is CENSORED SESSION_END, stamped at the close; ignore: no session boundary.", "truncate is how 'observe until the session ends' is expressed: under censor a horizon longer than the session censors every candidate, so time-to-event past a fixed horizon cannot be measured at all.", "session_end: truncate"),
    "outcome.session": ("Censoring session (default population.session): RTH, ETH, ALL, or TRADING_DAY (calendar datasets only: the dataset's own session row, e.g. the Globex trading day of NQ_1S_V2_GLOBEX). Use with population.session ALL to censor a whole-tape census at the RTH close or at the trading-day close.", "None.", "session: TRADING_DAY"),
    "outcome.cost_points_per_side": ("Round-trip cost basis for the C1 terminal block: points charged per side (entry and exit). Only read when the outcome carries the canonical terminal (terminal_outcome).", "Absent (default): terminal_cost_points / terminal_net_pnl_points / terminal_net_pnl_atr are NULL -- gross is canonical and net is explicitly unavailable, never guessed. Declared: net = gross - 2 * cost_points_per_side.", "cost_points_per_side: 0.25"),
    "outcome.entry_observation": ("Opt-in causal executable-entry observation: executable_entry_ts / executable_entry_price / executable_entry_unavailable_reason. The entry is the OPEN of the first execution bar with ts_init strictly after T (entry_ts = that bar's open instant, the same convention as terminal_entry_ts and the arms' resolution_seconds anchor), published whenever it is known -- independent of any exit, flip, arm or terminal state.", "Needs barrier arms (barrier or composite kernel). NULL with a reason, never fabricated: SESSION_END (the first bar after T closes past T's session close), GAP (it opens more than max_gap after T), DATA_END (no bar after T). terminal_entry_* keeps its C1 contract (published only beside an executable exit). Absent (default): no columns, the plan replays unchanged.", "entry_observation: true"),
    ...
    "analysis.source": ("Which collected partition set the analysis frame comes from: 'train' or 'oos'.",
                        "source: oos opens the protected period through assert_oos_open and needs chronology.dev years; source: train never opens one.",
                        "source: train"),
    "analysis.steps": ("Ordered pipeline of registered analysis operations (`research cap list analysis_ops`).",
                       "Steps run in declaration order and may only read the study frame or an EARLIER step, so the pipeline is a DAG proven at compile time.",
                       "steps: [{id: anchors, op: analysis.anchor.first_threshold_crossing, params: {...}}]"),
    "analysis.steps[].id": ("Step name; later steps and artifacts refer to it.", "None.", "id: anchors"),
    "analysis.steps[].op": ("Registered analysis_ops capability id.", "None.", "op: analysis.incidence.cumulative"),
    "analysis.model_scores": ("Join the scores of THIS study's own fitted model(s) into every analysis frame as `score__<record>` columns (`score__primary` for a single fit, `score__<arm>:<cell>` otherwise).",
                              "Each record is authenticated against the canonical bytes its TRAIN freeze committed to before scoring; a study that fits no model (model: none, or mode: score) is refused -- a frozen external model's scores are a derived input, never model_scores.",
                              "model_scores: true"),
    "analysis.steps[].rows": ("Primary frame input: 'frame' (the collected study frame), 'train_frame' (the TRAIN partition; source: oos only) or an earlier step id.", "A forward reference is UNSUPPORTED_COMPOSITION; 'train_frame' under source: train is refused, not aliased -- a tail evaluated at its own quantile is a description, not a test.", "rows: anchors"),
    "analysis.steps[].inputs": ("Extra frame inputs the operation declares, mapped to 'frame', 'train_frame' (source: oos only) or an earlier step id.", "Missing or unknown inputs are typed gaps at compile time. `inputs: {reference: train_frame}` freezes tail_lift thresholds on TRAIN scores and applies them to the OOS rows.", "inputs: {reference: train_frame}"),
    "analysis.steps[].params": ("The operation's declared parameters -- the science lives here, never in code.", "None.", "params: {horizons: [60, 180, 300]}"),
    "analysis.artifacts": ("The files this analysis produces under studies/<id>/artifacts.", "An analysis that declares no artifact is refused: it could not be audited.", "artifacts: [{name: incidence.json, source: incidence}]"),
    "analysis.artifacts[].name": ("Plain .json / .parquet / .md file name.", "None.", "name: first_p90_cumulative_incidence.json"),
    "analysis.artifacts[].source": ("The step whose output is written.", "None.", "source: incidence"),
    "analysis.artifacts[].kind": ("json (the step payload), frame (its output frame), or observations (its long-form observation frame).", "None.", "kind: frame"),
    ...
    "model.target": ("THE MODEL TARGET, derived at modeling time from columns the collected frame already carries: `{id, expr}` in the bounded expression grammar of analysis.derive.columns (parsed, never eval-ed). Forward-outcome columns ARE legitimate target inputs -- a label resolves after the entry it describes -- and remain forbidden as model FEATURES.", "Absent (default): the fit label is outcome.label_column, unchanged. The expression must resolve to a boolean {0, 1, null}; referenced columns are proven against the plan at compile; the canonical AST hash is recorded in the modeling contract and the fit receipt records eligible/positive/negative/null counts. A single-class target is a hard failure, never a vacuous PASS. Deriving a target never alters a persisted partition.", "target: {id: A_terminal_winner, expr: 'terminal_gross_pnl_atr > 0'}"),
    "model.feature_columns": ("THE MODEL INPUT SURFACE when it is not the declared feature surface: an explicit, ordered list of columns this study ALREADY EMITS (a feature alias, a derived input, or a declared features.metadata column). Declaration order is the feature order.", "Absent (default): the model fits columns.features + columns.derived, unchanged. Validated per column and fail-closed -- an unknown column, an identity/provenance column (observation_ts, regime_start_ns, checkpoint_index), any observation/outcome column of this study's outcome contract (observation_columns, the label, per-arm <prefix>_label) and anything the forward-outcome guard rejects are all refused; there is no blanket 'metadata is safe' rule. It selects MODEL INPUTS only: it changes no observation value and no partition identity, so an already-collected partition stays reusable (the reuse key excludes `model`).", "feature_columns: [dir_5m, dir_15m, dir_1h, age_s_5m, prior_mfe_atr_1h]"),
    ...
    "model.arms": ("Predeclared feature architectures, each fit independently on the same collected population.",
                   "A BARE list of names is refused in train mode (SEMANTIC_DECISION_REQUIRED): it used to compile and silently train ONE model on the whole surface. Declare {id, features, baseline}. Exactly one arm must be baseline: true.",
                   "arms: [{id: A, baseline: true, features: [x, y]}, {id: B, features: [x, y, z]}]"),
    "model.arms[].id": ("Arm name; appears in the model lineage as target_arm.", "Unique across arms.", "id: BASELINE"),
    "model.arms[].features": ("This arm's own subset of the declared feature surface.", "Every entry must be a declared feature or derived column; empty means the full surface.", "features: [arrival_velocity, ema_slope]"),
    "model.arms[].params": ("Arm-level hyperparameter override.", "Merged over model.params and the cell's params.", "params: {n_estimators: 200}"),
    "model.arms[].baseline": ("Marks the control arm every other arm is compared against.", "Exactly one arm must set it; paired deltas are computed against it.", "baseline: true"),
    "model.cells": ("Population cells fit independently, each with its own fixed hyperparameters (typically a direction).",
                    "Cells partition by an explicit column == value filter; there are no hidden direction semantics.",
                    "cells: [{id: LONG, subset: {regime_direction: 1}, params: {n_estimators: 100}}]"),
    "model.cells[].id": ("Cell name; appears in the model lineage as cell_id.", "Unique across cells.", "id: LONG"),
    "model.cells[].subset": ("Explicit column == value row filter selecting this cell's rows.", "Every column must be one the study emits.", "subset: {regime_direction: 1}"),
    "model.cells[].params": ("This cell's fixed hyperparameters.", "Merged over model.params; an arm's params win over a cell's.", "params: {learning_rate: 0.0394, n_estimators: 100}"),
    "model.validation.fold_months_year": ("The single calendar year rolling monthly folds walk across.", "Must be a declared TRAIN year; a dev or prohibited year is refused.", "fold_months_year: 2024"),
    "model.validation.train_months": ("Width in months of each fold's fit window.", "walk_forward_months only.", "train_months: 3"),
    "model.validation.validate_months": ("Width in months of each fold's validation window.", "walk_forward_months only; always strictly after the fit window.", "validate_months: 1"),
    "model.validation.step_months": ("How far the window advances between folds.", "walk_forward_months only.", "step_months: 1"),
    "model.reference_models": ("Frozen models scored ALONGSIDE the arms this study trains, on the identical population.",
                               "Never refit. mode stays train. Without this a descriptive reference needs its own study, and then it is not the same rows.",
                               "reference_models: [{id: <sha256>, name: PARENT_LONG, label: target_flip_within_horizon, subset: {regime_direction: 1}}]"),
    "model.reference_models[].id": ("Model-store id of the frozen model.", "64 hex characters.", "id: ccd587df..."),
    "model.reference_models[].label": ("Outcome column the reference is evaluated against.", "Must be an outcome column of this study.", "label: target_flip_within_horizon"),
    "model.reference_models[].subset": ("Explicit column == value row filter for this reference.", "None.", "subset: {regime_direction: 1}"),
    "model.reference_models[].name": ("Reporting name for the reference.", "Defaults to the id prefix.", "name: PARENT_LONG_2021_2023"),
    "model.reference_models[].expect": ("Identity expectations authenticated against the model-store lineage before scoring.", "A mismatch refuses the scoring.", "expect: {study_id: parent_study}"),
    "model.reference_models[].expect.study_id": ("Expected originating study id.", "None.", "study_id: clean_maturity_flip_model_180s_horizon"),
    "model.reference_models[].expect.target_arm": ("Expected target arm on the frozen lineage.", "None.", "target_arm: C"),
    "model.reference_models[].expect.direction": ("Expected direction on the frozen lineage.", "None.", "direction: LONG"),
    "model.reference_models[].expect.cell_id": ("Expected cell id on the frozen lineage.", "None.", "cell_id: LONG_C"),
    "model.reference_models[].expect.canonical_sha256": ("Expected canonical estimator BYTES.", "Catches a substituted estimator that refreshed its own bytes under an unchanged model_id.", "canonical_sha256: f969037b..."),

...
MEANING: Dict[str, Tuple[str, str, str]] = {
    ...
    "analysis.steps[].gate": ("Run this step as a PRE-FIT gate at the population_parity stage instead of at analyze.",
                              "A gate runs on the reconciled candidate population BEFORE merge/fit, so a failure means no model is ever fit from that population. It must read 'frame' and can have no step inputs.",
                              "gate: true"),
    "analysis.steps[].op": ("Registered analysis_ops capability id.", "None.", "op: analysis.incidence.cumulative"),
    "analysis.steps[].rows": ("Primary frame input: 'frame' (the collected study frame) or an earlier step id.", "A forward reference is UNSUPPORTED_COMPOSITION.", "rows: anchors"),
    "analysis.steps[].inputs": ("Extra frame inputs the operation declares, mapped to 'frame' or an earlier step id.", "Missing or unknown inputs are typed gaps at compile time.", "inputs: {anchors: anchors}"),

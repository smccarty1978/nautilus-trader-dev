...
def test_explicit_instances_and_collection_universe_share_canonical_status():
    aliases = resolve_source_universe("verified_registry_numeric_universe")
    instances = (
    ...
    if canonical_definition_status("regime_efficiency") == "verified":
        resolved = resolve_feature_instances("verified_registry_numeric_universe", instances)
        assert [item["physical_alias"] for item in resolved] == ["prior_1m_regime_efficiency", "prior_5m_regime_efficiency"]
        assert set(item["physical_alias"] for item in resolved) <= set(aliases)
    else:
        ...
        with pytest.raises(FeatureInstanceError, match="UNVERIFIED_CANONICAL_FEATURE"):
            resolve_feature_instances("verified_registry_numeric_universe", instances)


def test_collection_resolver_is_deterministic_and_not_cache_backed():
    first = resolve_source_universe("verified_registry_numeric_universe")
    second = resolve_source_universe("verified_registry_numeric_universe")
    assert first == second

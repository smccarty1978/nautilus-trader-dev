# Model card — `long_bullish_flip_top25`

**Status: FINAL** · provenance `RECONSTRUCTED_NO_FITTED_ARTIFACT_EXISTED` · source of truth **joblib**

## Purpose

The selected long-side model (LONG_TOP25_SIGNAL_STRONG_PRESERVATION). 25 features, logistic regression - small enough to reimplement and audit directly inside NT.
...
(`feature_order_sha256 = d601abe692c78c04…`).
...
Hyperparameters: `{"penalty": "l2", "C": 1.0, "max_iter": 1000, "solver": "lbfgs", "random_state": 42, "imputer": "median", "scaler": "StandardScaler"}`
...
|---|---:|---:|
| AUC | 0.6729 | 0.64619 |
| Top-decile flip rate | 0.50887 | 0.54506 |
...
|---|---|---:|---:|---:|---|
| `joblib_reload_vs_source` | 2025 | 163,397 | 0.000e+00 | 1e-12 | **PASS** |
| `joblib_reload_vs_source` | 2026 | 52,488 | 0.000e+00 | 1e-12 | **PASS** |
| `auc_vs_published` | 2025 | 163,397 | 0.000e+00 | 1e-09 | **PASS** |
| `auc_vs_published` | 2026 | 52,488 | 0.000e+00 | 1e-09 | **PASS** |
| `manual_formula` | 2025 | 163,397 | 2.092e-13 | 1e-09 | **PASS** |

ONNX: `model.onnx` present, parity PASS (max_abs_diff 1.728e-07 vs joblib).
...
- `intercept.json` — `intercept = -1.1242977837958268`
...
model = joblib.load("artifacts/long_bullish_flip_top25/model.joblib")
order = pd.read_csv("artifacts/long_bullish_flip_top25/feature_order.csv")["feature_name"].tolist()
```

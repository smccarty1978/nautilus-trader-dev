...
> [!IMPORTANT]
> **VALIDATION SCOPE CLARIFICATION (AUDIT DATED 2026-09-16):**  
> This study is a **FROZEN-SIGNAL NAUTILUSTRADER EXECUTION ENGINE VALIDATION**, not a full end-to-end live streaming runtime validation.  
> The strategy consumed precomputed signal timestamps and exit targets from the frozen observational ledger (`counter_h0501_exit_ledger.parquet`) as an execution schedule to validate order submission, CME simulated matching, asynchronous fill latency, bid/ask slippage, commissions, and position lifecycle under NautilusTrader C-level `BacktestEngine`.  
> Live streaming feature extraction and live model evaluation were not performed inside the strategy during the replay.  
> **Audit Classification:** `EXECUTION_ENGINE_ONLY_FROZEN_SIGNAL_SCHEDULE`. Historical results and execution metrics below remain authoritative for execution mechanics.

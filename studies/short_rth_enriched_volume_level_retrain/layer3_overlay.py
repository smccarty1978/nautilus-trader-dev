...
offline), applies each (feature_set, model, retention-band) Layer 2
one-entry-per-regime schedule to the KNOWN fixed-807 regime set and reports
keep/drop/moved-entry counts plus economics on the kept subset using the
model's own selected checkpoint. Same overlay logic and semantic-cleanliness
argument as the prior retrain study's layer3_overlay.py (the fixed-807
regime set is a subset of regimes already present in this score-independent
surface).
"""
...
def overlay_one(schedule: pd.DataFrame, fixed807: pd.DataFrame) -> dict:
    ...
    added = sched_regimes - fixed_regimes
    ...
    for fs_name in FEATURE_SETS:
        for model_name in MODELS:
            ...
            for split_name, year in (("2025", 2025), ("2026", 2026)):
                for band in BANDS:
                    path = WORK / f"schedule_{key}_{split_name}_{band}.parquet"
                    if not path.exists():
                        continue
                    schedule = pd.read_parquet(path)
                    res = overlay_one(schedule, fixed807[year])
                    rows.append({"feature_set": fs_name, "model": model_name, "split": split_name,
                                "retention_band": band, **res})
    df = pd.DataFrame(rows)
    df.to_csv(RESULTS / "layer3_fixed807_overlay.csv", index=False)
    print(df[(df.split == "2025") & (df.retention_band == 0.35)].to_string(index=False))

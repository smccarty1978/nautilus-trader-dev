...
def load_top103_scores(years=(2024, 2025)) -> pd.DataFrame:
    model_dir = Path("studies/freeze_long_strict_models_v2/artifacts/LONG_STRICT_top103_gbt_v2")
    features = json.loads((model_dir / "feature_list.json").read_text())
    catalog=json.loads(Path("studies/freeze_long_strict_models_v2/production_catalog.json").read_text()); freeze=json.loads((model_dir/"freeze_manifest.json").read_text())
    import hashlib
    feature_hash=hashlib.sha256((model_dir/"feature_list.json").read_bytes()).hexdigest()
    if catalog["production_model"]!="LONG_STRICT_top103_gbt_v2" or catalog["models"]["LONG_STRICT_top103_gbt_v2"]["source_model_hash"]!=freeze["source_model_hash"] or freeze["deployment_status"]!="PRODUCTION" or hashlib.sha256((model_dir/"model.joblib").read_bytes()).hexdigest()!=freeze["source_model_hash"] or feature_hash!=freeze["files"]["feature_list.json"]: raise RuntimeError("production artifact provenance mismatch")
    model = joblib.load(model_dir / "model.joblib")
    rows=[]
    for year in years:
        parts=sorted(Path(f"studies/long_rth_strict_symmetric_retrain/_work/monthly/{year}").glob("*.parquet"))
        if len(parts)!=12: raise RuntimeError(f"{year}: incomplete strict monthly population")
        d=pd.concat([pd.read_parquet(p) for p in parts],ignore_index=True)
        d["score"]=model.predict_proba(d[features])[:,1]
        rows.append(d[["regime_start_ns","observation_time","score"]])
...
def load_1s_bars(year: int) -> pd.DataFrame:
    ...
    if (not df["ts_event"].is_monotonic_increasing or df["ts_event"].duplicated().any() or not np.isfinite(vals.to_numpy(float)).all() or (df.high<df.low).any() or (df.high<df[["open","close"]].max(axis=1)).any() or (df.low>df[["open","close"]].min(axis=1)).any() or (df.volume<0).any()): raise RuntimeError(f"invalid raw bars: {year}")
    # Exact parity policy: zero-volume and single-tick bars are retained.
    return df
...
def main():
    ...
    print(f"=== Pre-Flip Signal Reliability Study (legacy direction mode: {args.direction.upper()}) ===")
    print("Semantic identities: short=Bullish Fade; long=Bearish Fade (legacy CLI aliases deprecated)")
    
    out_dir = Path("studies/pre_flip_signal_reliability_top103/results")
    ...
    if args.direction in ["long", "all"]:
        ...
        strict_scores=load_top103_scores(); keys=["regime_start_ns","observation_time"]
        left=pd.MultiIndex.from_frame(df_long[keys]); right=pd.MultiIndex.from_frame(strict_scores[keys])
        if len(left)!=len(right) or not left.is_unique or not right.is_unique or set(left)!=set(right): raise RuntimeError("strict score/candidate population mismatch")
        old_dir=Path("studies/freeze_reduced_flip_model_artifacts/artifacts/long_bullish_flip_top25"); top25_scores=load_model_and_score(old_dir,df_long)
        df_long=df_long.merge(strict_scores,on=keys,how="left",validate="one_to_one")
        if df_long.score.isna().any() or len(df_long)!=len(left): raise RuntimeError("score merge attrition")
        common=df_long[keys+["confirm_flip_ns"]].copy(); common["top25_score"]=top25_scores; common["top103_score"]=df_long.score; common.to_parquet(out_dir/"common_checkpoint_scores.parquet",index=False)
        df_top25=df_long.copy(); df_top25["score"]=top25_scores
        evaluate_direction(df_top25,"long",bars_dict,percentiles).to_csv(out_dir/"signal_population_top25_recomputed.csv",index=False)

"""Phase 1 (part 1): run ALL_FLIPS through NT BacktestEngine for every
year needed by the study's chronological splits, writing raw per-year
NT output to _work/nt_raw/<year>/ (trades.parquet, checkpoints.parquet,
diag.json). This is CACHE, not a final deliverable -- build_atlas.py
reads from here to construct the labeled checkpoint atlas in results/.
...
# 2026: only through the catalog's last available date (see
# audit/population_definition.md and the catalog reconciliation --
# data/catalog/NQ_v0_2020_2026 covers through 2026-04-30).
...
def main():
    ...
    for year in years_to_run:
        ...
        print(f"\n[{year}] ALL_FLIPS collection -> {out_dir}", flush=True)
        t0 = time.time()
        if year == 2026:
            load_start = (pd.Timestamp("2026-01-01", tz="UTC")
                             - pd.Timedelta(days=5))
            res = run_period(AllFlipsStrategy, AllFlipsConfig, {},
                                 load_start, YEAR_2026_END, out_dir)
        else:
            res = run_year(AllFlipsStrategy, AllFlipsConfig, {}, year,
                               out_dir)

"""Phase 3 driver for ALL_FLIPS: train the W0 (local progress/giveback
only) weakness model.

SCOPE NOTE (user-confirmed 2026-07-11): W1 (median-center context),
W2 (regime-sequence context), W3, and W4 require substantial fresh
feature engineering (~180 features causally recomputed at this
study's own 1s-cadence, entry-anchored checkpoint grid -- the old
atlas's precomputed values cannot be reused, see
audit/train_serve_skew_report.md). Per explicit user direction, this
study proceeds W0-only through Phases 4-8 to get a complete, validated
end-to-end skeleton and a first real answer to the core hypothesis;
W1-W4 are backfilled afterward if warranted.
"""
...
def main():
    ...
    # Feature-family ablation: only W0 exists today. Written as a
    # 1-row (per split) table with an explicit note column so the
    # required deliverable exists and future W1-W4 rows append here.

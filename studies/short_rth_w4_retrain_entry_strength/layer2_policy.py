...
For each (model, retention band, split): for every bullish regime, select
the first eligible RTH checkpoint (earliest observation_time) whose
stop_survival_score clears that split's frozen cutoff (cutoffs are computed
on 2025 dev and applied unchanged to 2026; train uses its own cutoffs,
diagnostic only). This produces a comparable candidate-entry schedule,
directly comparable to the current W4 candidate basis (Baseline A).

Consumes `train_and_evaluate.py`'s scored parquets and frozen cutoffs.
Trains/selects nothing new.
"""
...
EXIT_REASONS = ("preflip_policy_stop", "confirmation_timeout_exit",
...
def main() -> None:
    ...
    schedules: dict[tuple, pd.DataFrame] = {}
    ...
    for model_name, score_col in (("logreg", "stop_survival_score_logreg"),
        ...
        eval_splits = list(splits.items()) if model_name != "w4_comparator" else \
            [("2025", splits["2025"]), ("2026", splits["2026"])]
        for split_name, path in eval_splits:
            df = pd.read_parquet(path)
            for band in BANDS:
                cutoff = cutoffs[model_name][str(band)] if str(band) in cutoffs[model_name] \
                    else cutoffs[model_name][band]
                schedule = build_schedule(df, score_col, cutoff)
                schedules[(model_name, split_name, band)] = schedule
                e = economics(schedule)
                econ_rows.append({"model": model_name, "split": split_name,
                                  "retention_band": band, "layer": "layer2_one_per_regime", **e})
                m = monthly(schedule)
                for _, r in m.iterrows():
                    monthly_rows.append({"model": model_name, "split": split_name,
                                         "retention_band": band, **r.to_dict()})
                ea = exit_attribution(schedule)
                for _, r in ea.iterrows():
                    exit_rows.append({"model": model_name, "split": split_name,
                                      "retention_band": band, **r.to_dict()})
    ...
    for (model_name, split_name, band), sched in schedules.items():
        out = WORK / f"schedule_{model_name}_{split_name}_{band}.parquet"
        sched.to_parquet(out, index=False, compression="zstd")

    print(pd.DataFrame(econ_rows).to_string(index=False))

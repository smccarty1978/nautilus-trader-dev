"""Phase 6 driver for F2_CONFIRMED: run every non-E0 policy in the
grid (E1, S1-S7 x {checkpoint, mfe} = 15 configs) over 2025 (full
year, sliced to dev_test = Mar-Dec at analysis time) and 2026 (through
the catalog's last available date, = the reserved_eval period in full).

E0 needs NO new run: it is identical to the already-collected Phase 1
baseline, just filtered to the dev_test/reserved_eval splits at
analysis time.
"""
...
from studies.f2_confirmed_exit_management.policy_strategy import (
    F2ConfirmedPolicyStrategy, F2ConfirmedPolicyConfig,
)
    ...
    for name in policy_names:
        ...
        for year in args.years:
            ...
            if year == 2026:
                ...
                res = run_year(
                    F2ConfirmedPolicyStrategy, F2ConfirmedPolicyConfig, {},
                    year, out_dir, strategy_post_init=post_init)
            elapsed = time.time() - t0
            print(f"  done in {elapsed:.0f}s, diag: {res['diag']}",
                     flush=True)

"""Phase 2 driver for F2_CONFIRMED: old (flip-close-anchored) vs
corrected (entry-fill-anchored) skew audit across the full 2021-2026
history.
"""
...
def main():
    ...
    report_lines = ["# F2_CONFIRMED Train/Serve (Old vs Corrected) Skew Audit\n"]
    report_lines.append(f"Built: {pd.Timestamp.utcnow()}\n")
    report_lines.append(
        "\n## entry_price_minus_flip_close_atr (property of the "
        "corrected atlas's own entries, no old-atlas dependency; NOTE "
        "for F2_CONFIRMED this measures entry_px against the ORIGINAL "
        "flip bar's close, one bar before the bar+1 confirmation that "
        "actually triggered entry -- expect a larger, systematic gap "
        "than ALL_FLIPS since a full confirmation bar's worth of price "
        "movement has occurred between flip and entry)\n")

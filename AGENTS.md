...
- **SESSION DISCIPLINE:** `WORKFLOW.md` §N -- one lifecycle phase per session with a `study handoff` card; STOP-AT-CAPABILITY-GAP (`CAPABILITY_GAP_HANDOFF.json`, fresh capability session on `chore/<topic>` after `ws chore claim`, fresh study session); `scripts/test_delta.py` instead of branch-vs-main suites; detached long jobs, no polling; context target <= ~50k tokens.
- **COMMIT GATE (supervisor / platform chores, `WORKFLOW.md` §N.3):** targeted tests only per commit; exactly ONE broad relevant `test_delta` run before merge to `main`. Never a broad suite before every commit.
...
deterministic artifacts, and resume from the correct lifecycle stage. Run the targeted test per
commit; the one broad `test_delta` run belongs before the merge (`WORKFLOW.md` §N.3 commit gate).
...
`implementer`, lifecycle execution to the governed controller (`scripts/run_governed_study.py`,
not an agent), and conclusions to `analysis-decider` — an agent that finds work outside its own column refers it in one line
and moves on.

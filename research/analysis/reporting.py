...
from research.analysis.metrics import (
    METRIC_DEFINITIONS, classification_bundle, descriptive_summary, economic_bundle,
)
from research.analysis.slices import (
    STUDY_FIXED_EDGES, FixedInterval, SliceResult, build_slices, slice_decile,
    slice_fixed_edges, slice_target_disposition,
)
...
def build_descriptive_table(
    ...
    *,
    ...
    """Single-row descriptive summary for one numeric feature column.

    Not a per-group slice table, so `reconciles_rows=False`: there is exactly one
    row and it is not a partition of `n_input_rows`.
    """
    res = descriptive_summary(values)
    row: Dict[str, Any] = {"column": column_name, "n": res.n, "n_missing": res.n_missing}
    row.update(res.extra)
    caveats = [] if res.computable else [f"descriptive summary not computable: {res.reason}"]
    return StandardTable(
        name=f"descriptive_{column_name}",
        rows=[row],
        slice_name="none",
        ...
        analysis_spec_sha256=analysis_spec_sha256,
        caveats=caveats,
        metric_definitions={"descriptive_summary": METRIC_DEFINITIONS["descriptive_summary"]},
        n_input_rows=int(len(values)),
        reconciles_rows=False,
    )


def build_target_disposition_table(
    y: pd.Series,
    meta: pd.DataFrame,
    *,
    ...
    """positive/negative slice of the declared target column.

    Delegates to `build_slice_table`, whose existing 'unassigned' row mechanism
    (H4) is what keeps censored/null target rows out of both 'positive' and
    'negative' instead of silently folding them into 'negative'.
    """
    sl = slice_target_disposition(meta, target_column=target_column)
    return build_slice_table(
        y, sl, scores=scores, name="by_target_disposition",
        dataset_identity_sha256=dataset_identity_sha256,
        analysis_spec_sha256=analysis_spec_sha256,
        extra_filters={"target_column": target_column},
    )


def build_fixed_edges_table(
    y: pd.Series,
    values: Sequence[float],
    intervals: Sequence[FixedInterval] = STUDY_FIXED_EDGES,
    *,
    ...
    """Declared fixed-edge bucket table -- never quantile/decile-derived."""
    sl = slice_fixed_edges(values, intervals, name="fixed_edges")
    return build_slice_table(
        y, sl, scores=scores, name="by_fixed_edges",
        dataset_identity_sha256=dataset_identity_sha256,
        analysis_spec_sha256=analysis_spec_sha256,
        extra_filters={"column": column_name, "edges": [iv.label for iv in intervals]},
    )

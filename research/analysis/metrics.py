...
METRIC_DEFINITIONS: Dict[str, str] = {
    ...
    "descriptive_summary": (
        "count_available, count_null, distinct_count, mean, std, min, max, plus the "
        "p10/p25/median/p75/p90 quantile ladder (delegated to quantiles())."
    ),
}
...
def descriptive_summary(values: Sequence) -> MetricResult:
    """count/null/distinct plus mean/std/min/max and the standard quantile ladder.

    The quantile ladder is delegated to `quantiles()` rather than recomputed, so a
    descriptive table and a standalone quantile table can never numerically disagree
    about the same column.
    """
    arr, missing = _clean(values, name="descriptive_summary")
    if arr.size == 0:
        return _not_computable("descriptive_summary", "no non-null values", 0, missing)
    extra: Dict[str, Any] = {
        "count_available": int(arr.size),
        "count_null": int(missing),
        "distinct_count": int(np.unique(arr).size),
        "mean": float(arr.mean()),
    ...
    q = quantiles(values)
    if q.computable:
        extra.update(q.extra)
    return MetricResult("descriptive_summary", None, n=int(arr.size), n_missing=missing, extra=extra)

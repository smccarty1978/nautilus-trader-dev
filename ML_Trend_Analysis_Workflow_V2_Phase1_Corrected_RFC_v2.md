...
**Status:** READY FOR CODING-AGENT IMPLEMENTATION AFTER THIS REVISION  
...
**No new top-level `StudySpec` field may be added in Phase 1.**
...
READINESS must assert that no alternate catalog opener exists on the representative study's governed execution surface.
...
Fail if the representative study's governed execution surface contains an alternate executable catalog path that bypasses `resolve_catalog_plan`.
...
Phase 1 must introduce an explicit output-surface contract capable of representing:

```text
collection candidate universe
```
...
The exact naming may differ, but the semantic distinction is mandatory.
...
Remove the duplicate hardcoded `declared_metadata` authority inside the representative collector where practical.

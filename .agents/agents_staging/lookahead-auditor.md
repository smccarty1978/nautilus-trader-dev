...
When the worker packet names a brief (`packets/<task_id>.brief.md`), the brief REPLACES the *Load first* reads above: it carries your exact checklist subset verbatim from `docs/CAUSAL_CHECKLIST.md`, the `docs/RESEARCH_WORKFLOW.md` §17 guarantees, the gate facts (preflight / readiness / tests / controller card), the closure files that changed since the prior audited composite, and the prior findings to adjudicate. Read exactly: brief -> audit packet -> this file. Then:
...
- **Use packet references first.** Streams, visibility, trackers, kernel, chronology, closure membership come from the packet and the brief; never compute closure membership yourself.
- **Inspect source only for a claim the packet cannot prove** (state flow, callback order, a write site); read the smallest range and cite `file:line`.
- **Stop after the checklist subset is satisfied**: every rule id lands under `Clean checks`, a finding, or `Not applicable`; then write the report and the card.
- **No speculative architecture findings**; a finding needs a concrete failure path.
- Do not read WORKFLOW.md, AGENTS.md, RESEARCH_WORKFLOW.md, PLATFORM_STATE.json or compiled_plan.json (only a specific field the packet omits). Do not run Python or shell variable assignments: the read-only allowlist denies them and each denial costs a turn. Keep `--notes` / `--next-action` free of `;` `|` `&` `>`.

The budget, severity rules, output contract and scope split above are unchanged; the brief bounds the reading, not the coverage.
